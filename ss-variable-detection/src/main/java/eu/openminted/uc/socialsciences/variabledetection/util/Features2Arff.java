...
public class Features2Arff
{
    ...
    public static void toArffFile(Mode mode, Dataset... datasets) throws IOException
    {
        for (Dataset dataset : datasets) {
            String path = GOLDSTANDARD_DIR + "/" + mode.toString().toLowerCase() + "/STS.gs."
        ...
        throws IOException
    {
        ...
        String fileName = MODELS_DIR + "/" + mode.toString().toLowerCase() + "/"
                + dataset.toString() + ".arff";
        File outFile = new File(fileName);
        // System.out.println(outFile.getAbsolutePath());

        Collection<File> files = FileUtils.listFiles(
                new File(FEATURES_DIR + "/" + mode.toString().toLowerCase() + "/" + dataset),
                new String[] { "txt" }, true);
    ...
    private static String toArffString(Collection<File> csvFiles,
            InputStream goldStandardInputStream)
        throws IOException
    {
        ...
        for (File file : csvFiles) {
            ...
            if (lines.size() == 0) {
                System.err.println(
                        "Empty feature file for " + feature + ". Experiment will probably fail.");
            }
            ...
            for (int doc = 1; doc <= lines.size(); doc++) {
                ...
                {
                    double value = Double.parseDouble(line); // There's just the score on the line,
                                                             // nothing else.
        ...
        if (goldStandardInputStream != null) {
            ...
            BufferedReader br = new BufferedReader(
                    new InputStreamReader(goldStandardInputStream, "UTF-8"));
